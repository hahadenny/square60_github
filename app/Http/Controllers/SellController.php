...
class SellController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($request->has('street_address')){
            ...
            $rules = [
                ...
                'maintenance' => in_array($request->type, array('6', '7')) ? 'required|numeric' : '',
            ...
            if(count($same_lists)) {
                return redirect('/home/listing/sell')->withInput()->with('status', '<span style="color:red;">Error: You already created the same listing before.</span>');
            }
                ...
                if (Auth::user()->isAdmin()){
                    ...
                    if($request->feature == 1 ){
                        ...
                        if (isset($request->id) && $request->id)
                            ...
                            return redirect('/home/listing')->with('status', "Listing ID: $addSell is created successfully! Please click the \"Non-Active\" button to activate your listing.");
                    }
                }
    ...
    public function delete(Request $request)
    {
        ...
        if(strtolower($request->submit) == 'delete' && isset($request->id)){
            ...
            if($list){
                if (Auth::user()->isAdmin()){
                    return redirect('/allsell')->with('status', "Listing ID: $request->id is deleted.");
                }else{
